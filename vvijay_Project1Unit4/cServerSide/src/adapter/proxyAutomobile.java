...
public static LinkedHashMap<Integer,Automobile> model=new LinkedHashMap<Integer,Automobile>();
static int l=0;	
public void BuildAutoFromPropertiesFile(Properties props,Automobile a){
		FileIO ff=new FileIO();
		int costint=100;
		a1 = new Automobile("name", costint);
		Automobile a1 = ff.buildAutoObjectFromPropertiesFile(props,a);
		model.put(l, a1);
		l=l+1;
		System.out.println("Building the linked hash map :- \n");
...
public void ShowAllAutosFromPropertiesFile(){
	for(int i=0;i<model.size();i++)
	{
		System.out.println("The elements are"+model.get(i));
	}
}
