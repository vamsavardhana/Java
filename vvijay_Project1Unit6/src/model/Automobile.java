...
public synchronized Automobile setPrices(){
	for(int i=0;i<5;i++)
	{
		...
		else if(this.opset.get(i).name.contains("Brakes"))
		{
			this.opset.get(i).setPriceOption(0, 0);
		...
		{
			this.opset.get(i).setPriceOption(0, 350);
			this.opset.get(i).setPriceOption(1, 0);
		}
		else if(this.opset.get(i).name.contains("Power Moonroof"))
		{
			this.opset.get(i).setPriceOption(0, 595);
			this.opset.get(i).setPriceOption(1, 0);
		}
		else
		{
...
//i++;
//}
	public ArrayList<Option> getoptnarray(OptionSet o){
		return o.getOpSetOptions();
	}
